...
async function performTranslation() {
    ...
    try {
        ...
        if (data.responseData && data.responseData.translatedText) {
            let translated = getBestTranslation(data, text);
            
            if (translated.startsWith("MYMEMORY WARNING")) {
                console.warn("MyMemory warning: ", translated);
            }
            ...
            if (sourceLang === "auto" && data.mtLangDet) {
                detectedLangCode = data.mtLangDet;
                const detectedName = languages[data.mtLangDet] || data.mtLangDet.toUpperCase();
                const autoOpt = sourceLangSelect.querySelector('option[value="auto"]');
                if (autoOpt) {
            ...
            // Populate alternative translation chips
            populateAlternatives(data, text, translated);
